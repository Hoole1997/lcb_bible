 ...
 * 实现DataReporter接口，用于向Firebase Analytics上报数据
 */
class FirebaseDataController : DataReporter {
    ...
    private fun getFirebaseAnalytics(): FirebaseAnalytics? {
        return try {
            val context = AnalyticsModuleProvider.Companion.getApplicationContext()
            if (context != null) {
    ...
    private fun mapToBundle(map: Map<String, Any>): Bundle {
        ...
        try {
            ...
            AnalyticsLogger.e("Map转Bundle失败: ${e.message}")
        }
        return bundle
    }
    ...
    private fun mapToJson(map: Map<String, Any>): String {
        return try {
            ...
            AnalyticsLogger.e("Map转JSON失败: ${e.message}")
            "{}"
        }
    }
    ...
    override fun reportData(eventName: String, data: Map<String, Any>) {
        try {
            ...
            if (analytics == null) {
                AnalyticsLogger.w("无法获取Firebase Analytics实例，跳过数据上报: $eventName")
                return
            }
            ...
            AnalyticsLogger.d("Firebase Analytics数据上报: $eventName, JSON: ${mapToJson(data)}")

        } catch (e: Exception) {
            AnalyticsLogger.e("Firebase Analytics数据上报失败", e)
        }
    }
    ...
    override fun setCommonParams(params: Map<String, Any>) {
        try {
            ...
            if (analytics == null) {
                AnalyticsLogger.w("无法获取Firebase Analytics实例，跳过设置公共参数")
                return
            }
            ...
            try {
                ...
                AnalyticsLogger.e("设置Firebase Analytics默认事件参数失败", e)
            }

            AnalyticsLogger.d("Firebase Analytics公共参数设置完成: ${mapToJson(params)}")

        } catch (e: Exception) {
            AnalyticsLogger.e("Firebase Analytics设置公共参数失败", e)
        }
    }
    ...
    override fun setUserParams(params: Map<String, Any>) {
        try {
            ...
            if (analytics == null) {
                AnalyticsLogger.w("无法获取Firebase Analytics实例，跳过设置用户参数")
                return
            }
            ...
            params.forEach { (key, value) ->
                try {
                    ...
                    AnalyticsLogger.e("设置Firebase Analytics用户属性失败: $key = $value", e)
                }
            }

            AnalyticsLogger.d("Firebase Analytics用户参数设置完成: ${mapToJson(params)}")

        } catch (e: Exception) {
            AnalyticsLogger.e("Firebase Analytics设置用户参数失败", e)
        }
    }
}