 ...
 * 将广告收益数据上报到Adjust平台
 */
class AdjustAdRevenueReporter : AdRevenueReporter {
    ...
    private fun loadRevenueConfig() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val remoteConfigJson = RemoteConfigManager.getString("rev_adj", "")
                ...
                if (remoteConfigJson != null && remoteConfigJson.isNotEmpty()) {
                    ...
                    AnalyticsLogger.d("从Remote Config加载收益配置成功: $revenueConfigs")
                } else {
                    ...
                    if (assetsConfigJson != null) {
                        ...
                        AnalyticsLogger.d("从assets加载收益配置成功: $revenueConfigs")
                    } else {
                        // 如果assets文件也读取失败，使用硬编码默认配置
                        revenueConfigs = getDefaultConfig()
                        AnalyticsLogger.d("使用硬编码默认收益配置: $revenueConfigs")
                    }
                }
    ...
    private fun loadConfigFromAssets(): String? {
        return try {
            ...
            if (context != null) {
                ...
                AnalyticsLogger.w("无法获取Context，跳过从assets读取配置")
                null
            }
        } catch (e: IOException) {
            AnalyticsLogger.e("从assets读取revenue_config.json失败", e)
            null
        } catch (e: Exception) {
            AnalyticsLogger.e("读取assets配置异常", e)
            null
        }
    }
    ...
    private fun selectRevenueConfig(): String {
        if (!isConfigLoaded || revenueConfigs.isEmpty()) {
            AnalyticsLogger.w("收益配置未加载或为空，使用默认值")
            return "admob_sdk"
        }
        ...
        for (config in revenueConfigs) {
            ...
            if (randomValue <= cumulativeRate) {
                AnalyticsLogger.d("随机数: $randomValue, 选中配置: ${config.name}")
                return config.name
            }
        }
        ...
        AnalyticsLogger.d("随机数: $randomValue, 未匹配到配置，使用最后一个: ${lastConfig.name}")
     ...
     * @return 是否初始化成功
     */
    private suspend fun waitForAdjustInitialization(): Boolean {
        var attempts = 0
        val maxAttempts = 10 // 最多尝试10次
        val delayMs = 100L // 每次等待100ms

        while (attempts < maxAttempts) {
            if (AdjustController.isInitialized()) {
                return true
            }
            
            AnalyticsLogger.d("Adjust SDK未就绪，等待中... (${attempts + 1}/$maxAttempts)")
            delay(delayMs)
            attempts++
        }
        
        AnalyticsLogger.e("等待Adjust SDK初始化超时")
        return false
    }
    ...
    override fun reportAdRevenue(adRevenueData: AdRevenueData) {
        try {
            // 先检查Adjust SDK是否已初始化
            var isInitialized = AdjustController.isInitialized()
            
            // 如果没有初始化，则异步等待阻塞获取
            if (!isInitialized) {
                AnalyticsLogger.d("Adjust SDK未就绪，开始异步等待...")
                isInitialized = runBlocking {
                    waitForAdjustInitialization()
                }
            }
            
            if (!isInitialized) {
                AnalyticsLogger.w("无法等待Adjust SDK初始化完成，跳过广告收益上报")
                return
            }
            ...
            AnalyticsLogger.d("广告收益数据已上报到Adjust: $adRevenueData, 使用配置: $selectedConfigName")
            
        } catch (e: Exception) {
            AnalyticsLogger.e("上报广告收益数据到Adjust失败", e)
        }
    }
}
